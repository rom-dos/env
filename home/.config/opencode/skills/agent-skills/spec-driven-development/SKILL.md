...
IMPORTANT: Ask the human if there is a relevant Jira ticket for the work. If there is a `{response}` other than "no", run the following bash command to retrieve the ticket details from Jira, where `{response}` is the user's response:

```bash
acli jira workitem view {response}
```
